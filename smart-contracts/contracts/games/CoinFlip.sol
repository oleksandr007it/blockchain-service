...
contract CoinFlip is BaseGame {
    ...
    function placeBetSlip(
        ...
        if (
            ...
            keccak256(abi.encodePacked(("HEAD")))
        ) {
            gameChoice = keccak256(abi.encodePacked(("HEAD")));
        } else if (
            keccak256(abi.encodePacked((playerChoice))) ==
            keccak256(abi.encodePacked(("TAIL")))
        ) {
            gameChoice = keccak256(abi.encodePacked(("TAIL")));
        } else {
    ...
    function revealSeed(string memory seedHash, string memory seed) public {
        ...
        string memory gameResult = getCoinFlipGameResult(
            resultNumber
        );
    ...
    function getReturnAmount(
        ...
        if (playerChoice == keccak256(abi.encodePacked(("HEAD")))) {
            if (resultNumber > 0) {
                returnAmount = (wagerAmount * odds) / 100;
            } else {
                returnAmount = 0;
            }
        } else if (playerChoice == keccak256(abi.encodePacked(("TAIL")))) {
            if (resultNumber == 0) {
                returnAmount = (wagerAmount * odds) / 100;
            } else {
                returnAmount = 0;
    ...
    function getCoinFlipGameResult(uint256 resultNumber)
    private
    pure

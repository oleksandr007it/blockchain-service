...
async function main() {
  ...
  const coinflip = await CoinFlip.deploy(betSlips.address, 97);
  await coinflip.deployed();

  await betSlips.grantGameRole(coinflip.address);
  await betSlips.grantCompleterRole(coinflip.address);

  console.log("CoinFlip Address: ", coinflip.address);

  const tx = await coinflip.setBetLimit(USDT_ADDRESS, BigNumber.from(100000), BigNumber.from(10000000), BigNumber.from(2000000));
  await tx.wait();
}

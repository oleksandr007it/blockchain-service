...
describe("DiceGame", () => {
  ...
  describe("Set betlimt and get game config", () => {
    it("set betlimit and get game config", async () => {
      await expect(diceGame.setBetLimit(USDT.address, 20, 10000, 32))
        .to.emit(diceGame, 'betLimitChangedEvent')
        .withArgs('dice', 'USDT', USDT.address, 20, 10000, 32);

      expect(await diceGame.getGameConfig(USDT.address)).to.equal(`{"rtp":97,"betLimits": {"${USDT.address.toLowerCase()}": {"minAmount": 20,"maxAmount": 10000,"defaultAmount": 32}}}`);

      tx = await diceGame.setBetLimit(USDT.address, 200, 20000, 50);
      await tx.wait();

      expect(await diceGame.getGameConfig(USDT.address)).to.equal(`{"rtp":97,"betLimits": {"${USDT.address.toLowerCase()}": {"minAmount": 200,"maxAmount": 20000,"defaultAmount": 50}}}`);
    });
  });
  ...
  describe("debit and place, complete the BetSlip without permit", () => {
    it("When paused, place betSlip", async () => {
      ...
      await expect(diceGame.pauseGame())
        .to.emit(diceGame, 'gameStateChangedEvent')
        .withArgs('dice', false);

      await expect(diceGame.placeBet(200, 42, "OVER", seedHash, USDT.address))
        .to.be.revertedWith("Pausable: paused");

      await expect(diceGame.unpauseGame())
        .to.emit(diceGame, 'gameStateChangedEvent')
        .withArgs('dice', true);
    })
    ...
    it("validate the bet", async () => {
      ...
      await expect(diceGame.placeBet(200, 42, "OVERUNDER", seedHash, USDT.address))
        .to.be.revertedWith("The choice is invalid");

      await expect(diceGame.placeBet(50000, 42, "OVER", seedHash, USDT.address))
        .to.be.revertedWith("The WagerAmount is invalid");

      await expect(diceGame.placeBet(10, 42, "UNDER", seedHash, USDT.address))
        .to.be.revertedWith("The WagerAmount is invalid");
    })
    ...
    it("debit the token amount", async () => {
      ...
      tx = await diceGame.placeBet(200, 42, "OVER", seedHash, USDT.address);
      await tx.wait();
    ...
    it("place the betslip", async () => {
      ...
      await expect(diceGame.placeBet(200, 42, "OVER", seedHash, USDT.address))
        ...
        .withArgs(2, owner.address, USDT.address, "dice", '{"playerChoice":"OVER", "playerNumber":42}', 200, seedHash, 170, 0);
    ...
    it("complete the betslip valid seed", async () => {
      ...
      const placeTx = await diceGame.placeBet(200, 42, "OVER", seedHash, USDT.address);
      await placeTx.wait();
      ...
      await expect(diceGame.revealSeed(seedHash, input_str))
        ...
        .withArgs(3, owner.address, USDT.address, 'dice', '{"playerChoice":"OVER", "playerNumber":42}', 200, seedHash, "84", 340, input_str, 170, 1);
  ...
  describe("revoke betslip contract", () => {
    it("revoke betslips", async () => {
      ...
      const placeTx1 = await diceGame.placeBet(200, 42, "OVER", seedHash1, USDT.address);
      await placeTx1.wait();
      ...
      const placeTx2 = await diceGame.placeBet(200, 42, "OVER", seedHash2, USDT.address);
      await placeTx2.wait();
      const completeTx1 = await diceGame.revealSeed(seedHash2, input_str2);
      ...
      const placeTx4 = await diceGame.placeBet(200, 42, "OVER", seedHash3, USDT.address);
      await placeTx4.wait();
      await expect(diceGame.revealSeed(seedHash2, input_str3))
        .to.be.revertedWith("Invalid seed");
      ...
      await expect(diceGame.placeBet(200, 42, "OVER", seedHash1, USDT.address))
        .to.be.revertedWith("SeedHash is already used");                  // When betslips is revoked, placeBet
      await expect(diceGame.revealSeed(seedHash2, input_str2))
        .to.be.revertedWith("Betslip is already terminated.");            // When betslips is completed, revealSeed
      await expect(diceGame.revealSeed(seedHash3, input_str3))

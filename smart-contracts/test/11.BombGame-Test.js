...
describe("Bomb Game", () => {
    ...
    before(async () => {
        ...
        bombGame = await BombGame.deploy(betSlips.address, 97);
        await bombGame.deployed();

        await betSlips.grantGameRole(bombGame.address);
        await betSlips.grantCompleterRole(bombGame.address);
    });

    it("is deployed", async () => {
        expect(await bombGame.getBetSlipsAddress()).to.equal(betSlips.address);
    ...
    describe("Set and Get Rtp", () => {
        it("set and get Rtp", async () => {
            await expect(bombGame.setRtp(95))
                .to.emit(bombGame, 'rtpChangedEvent')
                .withArgs('bomb', 95);
            expect(await bombGame.getRtp()).to.equal(95);
        
            await expect(bombGame.setRtp(97))
                .to.emit(bombGame, 'rtpChangedEvent')
                .withArgs('bomb', 97);
            expect(await bombGame.getRtp()).to.equal(97);
        });
    });
    
    describe("Set betlimt and get game config", () => {
        it("set betlimit and get game config", async () => {
            await expect(bombGame.setBetLimit(USDT.address, 20, 10000, 32))
                .to.emit(bombGame, 'betLimitChangedEvent')
                .withArgs('bomb', 'USDT', USDT.address, 20, 10000, 32);
        
            expect(await bombGame.getGameConfig(USDT.address)).to.equal(`{"rtp":97,"betLimits": {"${USDT.address.toLowerCase()}": {"minAmount": 20,"maxAmount": 10000,"defaultAmount": 32}}}`);
        
            tx = await bombGame.setBetLimit(USDT.address, 200, 20000, 50);
            await tx.wait();
        
            expect(await bombGame.getGameConfig(USDT.address)).to.equal(`{"rtp":97,"betLimits": {"${USDT.address.toLowerCase()}": {"minAmount": 200,"maxAmount": 20000,"defaultAmount": 50}}}`);
        });
    });
    ...
    describe("debit and place, complete the BetSlip without permit", () => {
        it("When paused, place betSlip", async () => {
            ...
            await expect(bombGame.pauseGame())
              .to.emit(bombGame, 'gameStateChangedEvent')
              .withArgs('bomb', false);
      
            await expect(bombGame.placeBet(200, "GREEN", seedHash, USDT.address))
              .to.be.revertedWith("Pausable: paused");
      
            await expect(bombGame.unpauseGame())
              .to.emit(bombGame, 'gameStateChangedEvent')
              .withArgs('bomb', true);
        })
        ...
        it("validate the bet", async () => {
            ...
            await expect(bombGame.placeBet(50000, "RED", seedHash, USDT.address))
                .to.be.revertedWith("The WagerAmount is invalid");

            await expect(bombGame.placeBet(10, "BLUE", seedHash, USDT.address))
                .to.be.revertedWith("The WagerAmount is invalid");
        })


        it("debit the token amount bomb game", async () => {
            let input_str = "8cf186a0e38d626c236527296c8e92adbc51fa06961f66b83f3e0d335dd5019e";
            ...
            tx = await bombGame.setBetLimit(USDT.address, 1, 20000, 50);
            await tx.wait();

            await expect(bombGame.placeBet(200, "BLUE", seedHash, USDT.address))
                .to.be.revertedWith("Insufficient allowance");

            tx = await USDT.approve(betSlips.address, 2000);
            await tx.wait();

            tx = await bombGame.placeBet(20, "RED", seedHash, USDT.address);
            await tx.wait();
        ...
        it("player win with valid blue seed for bomb game", async () => {
            let blue_seed = "7df5bee0e1f04caa3b68ecf56fe9c3c7bfece75f44ff2bf448cf516e7c7ce0ab";
            ...
            const placeTx = await bombGame.placeBet(200, "BLUE", seedHash, USDT.address);
            await placeTx.wait();
            ...
            await expect(bombGame.revealSeed(seedHash, blue_seed))
                .to.emit(betSlips, 'betSlipCompleted')
                .withArgs(2, owner.address, USDT.address, 'bomb', 'BLUE', 200, seedHash, 'BLUE', 388, blue_seed, 194, 1);
        ...
        it("player win with valid red seed for bomb game", async () => {
            let red_seed = "3d4a04e1caafeb597c1c5e9bd9c2704b18153786c081c6d1c81aab9c42679910";
            ...
            const placeTx = await bombGame.placeBet(200, "RED", seedHash, USDT.address);
            await placeTx.wait();
            ...
            await expect(bombGame.revealSeed(seedHash, red_seed))
                .to.emit(betSlips, 'betSlipCompleted')
                .withArgs(3, owner.address, USDT.address, 'bomb', 'RED', 200, seedHash, 'RED', 388, red_seed, 194, 1);
        ...
        it("player lose the bomb game", async () => {
            let blue_seed = "66953fd3a4824a9d38c893b633564470b6cfd0a48d6655773e7246423ae519e7";
            ...
            await expect(bombGame.revealSeed(seedHash, blue_seed))
                .to.emit(betSlips, 'betSlipCompleted')
                .withArgs(4, owner.address, USDT.address, 'bomb', 'RED', 200, seedHash, 'BLUE', 0, blue_seed, 194, 1);
        ...
        it("complete the betslip with invalid seed", async () => {
            ...
            await expect(bombGame.revealSeed(1 , input_str))
    ...
    describe("revoke betslip contract", () => {
        it("revoke betslips", async () => {
            ...
            const placeTx1 = await bombGame.placeBet(200, "RED", seedHash1, USDT.address);
            await placeTx1.wait();
            ...
            const placeTx2 = await bombGame.placeBet(200, "RED", seedHash2, USDT.address);
            await placeTx2.wait();
            const completeTx1 = await bombGame.revealSeed(seedHash2, input_str2);
            ...
            const placeTx4 = await bombGame.placeBet(200, "RED", seedHash3, USDT.address);
            await placeTx4.wait();
            await expect(bombGame.revealSeed(seedHash2, input_str3))
                .to.be.revertedWith("Invalid seed");
            ...
            await expect(bombGame.placeBet(200, "RED", seedHash1, USDT.address))
                .to.be.revertedWith("SeedHash is already used");                  // When betslips is revoked, placeBet
            await expect(bombGame.revealSeed(seedHash2, input_str2))
                .to.be.revertedWith("Betslip is already terminated.");            // When betslips is completed, revealSeed
            await expect(bombGame.revealSeed(seedHash3, input_str3))

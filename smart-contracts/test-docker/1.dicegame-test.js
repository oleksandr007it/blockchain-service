...
describe("DiceGame", () => {
  ...
  before(async () => {
    ...
    diceGame = await DiceGame.deploy(betSlips.address);
  ...
  describe("Set Rtp and Get Odds", () => {
    ...
    it("get odds", async () => {
      ...
      expect(await diceGame.getOdds(80, 0)).to.equal(510);
      expect(await diceGame.getOdds(50, 0)).to.equal(197);
  ...
  describe("Set betlimt and get game config", () => {
    it("set betlimit and get game config", async () => {
      ...
      expect(await diceGame.getGameConfig()).to.equal(`{"rtp":97,"betLimits": {"0xffffffff8d2ee523a2206206994597c13d831ec7": {"min": 20,"max": 10000,"default": 32},"0x96961d1440d6c38d3241b154865da7d06c971a2c": {"min": 20,"max": 10000,"default": 32}}}`);

      tx = await diceGame.setBetLimit(USDT_ADDRESS, 200, 20000, 50);
      await tx.wait();

      expect(await diceGame.getGameConfig()).to.equal(`{"rtp":97,"betLimits": {"0xffffffff8d2ee523a2206206994597c13d831ec7": {"min": 200,"max": 20000,"default": 50},"0x96961d1440d6c38d3241b154865da7d06c971a2c": {"min": 20,"max": 10000,"default": 32}}}`);
    });
  });
  ...
  describe("debit and place, complete the BetSlip without permit", () => {
    ...
    it("place the betslip", async () => {
      ...
      await expect(diceGame.placeBet(200, 42, "OVER", seedHash, USDT_ADDRESS))
        ...
        .withArgs(2, owner.address, USDT_ADDRESS, "dice", '{"playerChoice":"OVER", "playerNumber":42}', 200, seedHash, 170, 0);
    ...
    it("complete the betslip valid seed", async () => {
      ...
      await expect(diceGame.revealSeed(seedHash, input_str))
        ...
        .withArgs(3, owner.address, USDT_ADDRESS, 'dice', '{"playerChoice":"OVER", "playerNumber":42}', 200, seedHash, "84", 340, input_str, 170, 1);
